...
public:
  ...
  CustomLibModPass()
      : ModulePass(ID), chg(0), SafebcmpFnc(nullptr), SafebcmpFt(nullptr),
        ...
        SafememFt(nullptr), SaferandFnc(nullptr), SaferandFt(nullptr),
        SafemallocFnc(nullptr), SafemallocFt(nullptr), SafefreeFnc(nullptr),
        SafefreeFt(nullptr) {
...
bool CustomLibModPass::runOnModule(Module &M) {
  ...
  const char *randfns[] = {"random", "rand"};
  ...
  SafebzeroFnc =
      Function::Create(SafebzeroFt, Function::ExternalLinkage, "safe_bzero", M);
  SafememFt = FunctionType::get(VoidTy, SafememArgs, false);
  SafememFnc =
      Function::Create(SafememFt, Function::ExternalLinkage, "safe_mem", M);
  SaferandFt = FunctionType::get(Int64Ty, SaferandArgs, false);
  SaferandFnc =
  ...
  for (auto &F : M) {
    for (auto &BB : F) {
      ...
      for (auto bbbeg = BBlst.begin(), bbend = BBlst.end(); bbbeg != bbend;
           bbbeg++) {
        ...
        if (CI) {
          ...
          if (updateInst(FCI, CI, BBlst, bbbeg, SafememFnc))
            chg++;
          if (updateInst(FCI, CI, BBlst, bbbeg, SaferandFnc))
            chg++;
